...
def get_gpt2_finetune_model():
    ...
    model_path = os.path.join(path, "model/gpt2_finetune_first_last_all_20/")
...
def get_gradio_fn(first_sentence, last_sentence, max_len, num_return_sequences):
    max_len = int(max_len)
    num_return_sequences = int(num_return_sequences)
    if GPT2_FINETUNE_MODEL is not None and GPT2_FINETUNE_TOKENIZER is not None:
        seed_everything(2022)

        input_text = special_tokens['bos_token'] + first_sentence \
                     + special_tokens['sep_token'] + last_sentence + special_tokens['sep_token']
        ...
        for i, output in enumerate(generated_res):
            ...
            text_len = len(first_sentence) + len(last_sentence)
